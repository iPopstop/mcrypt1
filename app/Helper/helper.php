...
function parseDate($date, $quotes = true) {
    $info = Carbon::parse($date)->format('d.m.Y');
    return $quotes ? "\"{$info}\"" : $info;
}

function checkBool($value, $default = false)
{
    return isset($value)
...
function paginate($results, $pageSize) {
    return CollectionHelper::paginate($results, $pageSize);
}
...
function getDateFormat()
{
    ...
    if (config('config.date_format') === 'MM-DD-YYYY') {
        return 'm-d-Y';
    }

    if (config('config.date_format') === 'DD-MMM-YYYY') {
        return 'd-M-Y';
    }

    if (config('config.date_format') === 'MMM-DD-YYYY') {
        return 'M-d-Y';
    }
...
{
    return ip2long($ip) >= ip2long($range[0]) && ip2long($ip) <= ip2long($range[1]);
}
...
function getVar($list)
{
    ...
    return (\File::exists($file)) ? json_decode(file_get_contents($file), true, 512, JSON_THROW_ON_ERROR) : [];
}
